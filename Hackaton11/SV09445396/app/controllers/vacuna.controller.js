    ...
    Vacuna.findByPk(id)
        .then(data => {
            if (data) {
                ...
                res.status(404).send({
                    message: `No puede encontrar la vacuna con id=${id}.`
                });
            }
        })
        .catch(err => {
...
exports.update = (req, res) => {
    ...
    Vacuna.update(req.body, {
        ...
        .then(num => {
            if (num == 1) {
                res.send({
                    message: "Vacuna ha sido actualizado."
                });
            } else {
                res.send({
        ...
        .catch(err => {
            res.status(500).send({
                message: "Error actualizando vacuna con id=" + id
            });
        });
};
...
exports.delete = (req, res) => {
    ...
    Vacuna.destroy({
        ...
        .then(num => {
            if (num == 1) {
                res.send({
                    message: "Vacuna ha sido borrado correctamente."
                });
            } else {
                res.send({
                    message: `No se puede eliminar la vacuna con id=${id} Quizás no se encontró!`
                });
            }
        })
        .catch(err => {
            res.status(500).send({
                message: "No se pudo eliminar la vacuna con id=" + id
            });
        });
};
